...
namespace GCDTracker {
    ...
    public unsafe class GCDWheel {
        ...
        private class ProgressBarVertices {
            ...
            public ProgressBarVertices(BarInfo bar) {
                int rightClamp = (int)(bar.CenterX + ((bar.CurrentPos + bar.BorderWidthPercent) * bar.Width) - bar.HalfWidth);
                ...
                TL_C = new(                    
                    (int)(bar.CenterX + (bar.CurrentPos * bar.Width) - bar.HalfWidth),
                ...
                TR_C = new(
                    (int)(bar.CenterX + ((bar.CurrentPos + ((float)bar.BorderSizeAdj/ bar.Width)) * bar.Width) - bar.HalfWidth),
                    (int)(bar.CenterY - bar.RawHalfHeight)
                );
                TR_X = new(
                    rightClamp,
        ...
        private void DrawBarElements(PluginUI ui, bool isCastBar, bool isShortCast, float castBarCurrentPos, float gcdTime_slidecastStart, float gcdTotal_slidecastEnd) {
            ...
            var pb_v = new ProgressBarVertices(bar);
            ...
            if(bar.CurrentPos > 0.001f)
                ui.DrawRectFilledNoAA(bar.StartVertex, pb_v.BR_C, conf.frontCol);
